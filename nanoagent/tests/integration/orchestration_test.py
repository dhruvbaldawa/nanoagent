...
class TestManualOrchestration:
    """Integration tests for manual orchestration of planning → execution → reflection cycle"""
    ...
    async def test_complete_orchestration_cycle(self) -> None:
        ...
        # Phase 4: Executor - execute second task
        task_2 = todo.get_next()
        assert task_2 is not None, "TodoManager should return second task"
        assert task_2.id != task_1.id, "Second task should be different from first"
        ...
        assert len(completed_tasks) == 2, "Should have exactly 2 completed tasks"
        ...
        # Validate Risk #3: Reflection produces sensible output
        # If goal is complex, reflection should identify gaps or suggest new tasks
        # (exact behavior depends on LLM, but structure should always be valid)
        if not reflection_output.done:
            # If not done, should have identified gaps or suggested tasks
            assert len(reflection_output.gaps) > 0 or len(reflection_output.new_tasks) > 0, (
                "If not done, should identify gaps or suggest new tasks"
            )
    ...
    async def test_context_preservation_through_phases(self) -> None:
        ...
        # Execute second task and add to context
        task_2 = todo.get_next()
        ...
        assert len(context) == 2, "Context should have 2 entries"
