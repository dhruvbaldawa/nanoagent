...
class TestExecutor:
    """Test suite for Executor agent (uses real LLM calls with mock tools)"""
...
false if it failed. Provide detailed output explaining what you did.""",
        ...
        # Run executor
        result = await executor.run("Search for information about Python", deps=deps)  # type: ignore[arg-type]
    ...
    async def test_executor_returns_structured_output(self) -> None:
        ...
        result = await executor.run("Execute a simple task", deps=deps)  # type: ignore[arg-type]
